...
class EDataTables extends CGridView
{
	...
	public $widgetIcon = 'fa fa-file';
	public $pullIcons = array();
	public $hidePullIcons   = false;
	...
	public function run()
	{
		?>
			<div class="panel panel-white">
				...
				<?php if(count($this->pullIcons) > 0 && !$this->hidePullIcons) { ?>
	...
	{
		$id=$this->getId();
		if(!function_exists('visibleButton')){
    		function visibleButton($btn) {return !isset($btn['visible']) || $btn['visible'] != false; }
		}
		//filter out non visible buttons
		$buttons = array_filter($this->initToolbarButtons(), "visibleButton");
		...
		Yii::app()->getClientScript()->registerScript(__CLASS__.'baseScripts#'.$id, "
			$('#{$id} table').on('click', 'a.delete', function() {
				...
				$.ajax({
                    ...
                    if(typeof response =='object'){
                        if(response.status === false){
                            ...
                            $('#{$id} table').DataTable().row( $(this).parents('tr') ).remove().draw();
                        }
                    }else{
                        if(response ===false){
	...
	public function getFormattedData($sEcho) {
		...
		for($row=0; $row<$n; ++$row) {
			...
			foreach($this->columns as $column) {
				$count++;
				ob_start();
				echo $column->getDataCellContent($row);
